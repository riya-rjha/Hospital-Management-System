...
// Date may contain colons and dashes — split on first 3 underscores only
	...
	public BackendIdConverter onCallIdConverter() {
		...
		return new BackendIdConverter() {
			...
			public boolean supports(Class<?> delimiter) {
				...
				return OnCall.class.equals(delimiter);
			...
			public Serializable fromRequestId(String id, Class<?> entityType) {
				...
				if (id == null)
					...
					throw new IllegalArgumentException("OnCall id must not be null");

// Format: {blockFloor}_{nurse}_{blockCode}_{onCallStart}_{onCallEnd}
				...
				if (parts.length != 5)
					...
					throw new IllegalArgumentException(
							...
							"OnCall id must be {blockFloor}_{nurse}_{blockCode}_{onCallStart}_{onCallEnd}, got: " + id);

				return new OnCallId(

						Integer.parseInt(parts[1]), // nurse

						Integer.parseInt(parts[0]), // blockFloor

						Integer.parseInt(parts[2]), // blockCode

						java.time.LocalDateTime.parse(parts[3]), // onCallStart

						java.time.LocalDateTime.parse(parts[4]) // onCallEnd
			...
			public String toRequestId(Serializable id, Class<?> entityType) {
				...
				OnCallId oid = (OnCallId) id;

				return oid.getBlockFloor() + "_"

						+ oid.getNurse() + "_"

						+ oid.getBlockCode() + "_"

						+ oid.getOnCallStart().toString() + "_"

						+ oid.getOnCallEnd().toString();
